...
- [x] ~~**R2 螢趕上來場景缺圖**~~ v2.3.4h 已補 ASCII 肖像
- [x] ~~**R2 營火晚餐缺圖**~~ v2.3.4h 已補 18 行營火 ASCII art
- [x] ~~**R3 偷東西小孩缺圖**~~ v2.3.4h 已補 14 行市場攤位 ASCII art

#### 節點流程錯誤（自動跳轉 bug）

- [x] ~~**鐵霜對話後自動進 Boss 戰**~~ v2.3.4g 修正節點流向回到 `r2_camp`
- [x] ~~**銅鐘對話後自動跳戰鬥區**~~ v2.3.4g 修正 `r3_bell` / `r3_quest_check` 尾端 action
- [x] ~~**R3 點擊自動進議會廳 + 跳回 R2 機甲**~~ v2.3.4g 修正 sub-hub loadNode 目標
- [x] ~~**R1 老周 bug**~~ v2.3.4g 放寬 `r1_survivor` 前置條件，確保選項露出

#### 文案 / UI 體驗

- [x] ~~「巡邏」改為「探索」~~ v2.3.4f 已完成大部分
- [x] ~~**「迴廊中巡邏練級」表述沉浸感不足**~~ v2.3.4i 改為區域感知的沉浸式標籤（深入陰影/潛行石脈/廢墟狩獵/河城暗處）
- [x] ~~**中樞選項禁用「巡邏練級」字眼**~~ v2.3.4h 全部改為沉浸式探索用語
- [x] ~~**通關後按鈕無法點擊**~~ v2.3.4g 修正 epilogue pending 狀態鎖住問題

#### R3 野怪遭遇設計

- [x] ~~**R3 河城不應有隨機野怪**~~ v2.3.4h 改為下水道/隧道/廢棄森林主動選擇才觸發戰鬥

#### 經濟系統 / 金幣繼承

- [x] ~~**金幣應限二周目以後獲得**~~ v2.3.4h 骰子賭博 + 金幣事件皆加 `ngPlus` 門檻
- [x] ~~**金幣可跨周目繼承**~~ v2.3.4h 新增 `globalStats.bankedGold`，NG+ 開局繼承
- [x] ~~**灰鶴的欠款應在 NG+ 歸零**~~ v2.3.4h 每次會面自動借本金，無需追蹤欠款

#### 灰鶴骰子系統 bug

- [x] ~~**灰鶴輸 60 金後就不賭了**~~ v2.3.4h 改為固定注額 + 自動借款，無上限
- [x] ~~**骰子羅曼史消失**~~ v2.3.4h 重寫 4 階 tiered 台詞系統，骰局自帶漸進式浪漫

#### R3 議會 Boss 風格

- [x] ~~**「御前守衛」Boss 設計**~~ v2.3.4h 重寫為御前守衛（鎧甲+石化長矛），全新 ASCII art + 戰鬥數值
...
- [x] ~~**周目排行榜**~~ v2.3.4k 排行榜顯示周目欄位，限制顯示前 10 名
