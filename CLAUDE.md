...
### 🐛 玩家測試回報 Bug 清單（2026-04 測試）

#### 美術資源缺失（需補 ASCII art / 像素圖）

- [ ] **R2 螢趕上來場景缺圖** — 螢在 R2 採石場追上玩家的節點（`r2_look` 或相關）需要加一張美術圖（可用 ying 像素肖像 + 場景 ASCII）
- [ ] **R2 營火晚餐缺圖** — `r2_camp_dinner` 場景需要營火＋餐具的 ASCII art
- [ ] **R3 偷東西小孩缺圖** — `r3_thief_kid` 隨機敘事事件需要小孩的 ASCII art 或像素肖像

#### 節點流程錯誤（自動跳轉 bug）

- [ ] **鐵霜對話後自動進 Boss 戰** — 跟鐵霜講完話後不應直接 `loadNode('r2_boss')`，應回到 `r2_look` 或營地中樞，讓玩家自主選擇進戰鬥時機
- [ ] **銅鐘對話後自動跳戰鬥區** — 跟銅鐘交談結束後節點流向錯誤，會自動跳到戰鬥區域（需查 `r3_bell` / `r3_quest_check` 的尾端 action）
- [ ] **R3 點擊自動進議會廳 + 跳回 R2 機甲** — 在 R3 中樞點擊時會自動觸發 `r3_quest_check`（回報銅鐘）然後 loadNode 跳回 `r2_machine`。可能是 sub-hub 的 choices 列表寫錯 loadNode 目標，或 quest_check 尾端誤寫 r2 節點 ID
- [ ] **R1 老周 bug** — 玩家在 R3 遇到老周（`r3_zhou`）但在 R1 沒觸發 `r1_survivor`。可能是 R1 中樞的老周分支有前置條件寫錯，或 `r1_look` 的選項沒正確露出 survivor 節點

#### 文案 / UI 體驗

- [x] ~~「巡邏」改為「探索」~~ v2.3.4f 已完成大部分
- [ ] **「迴廊中巡邏練級」表述沉浸感不足** — R1 status label 仍用「警戒探索迴廊」之類的字眼，應改成更有故事感的描述（例如「在迴廊深處搜尋生機」/「繼續潛行於石脈之間」）
- [ ] **中樞選項禁用「巡邏練級」字眼** — 檢查所有 `rN_look` / `rN_explore` 的選項文字，把剩餘的「巡邏/練級」都改掉
- [ ] **通關後按鈕無法點擊** — 結局後 UI 卡住，所有選項按鈕失效。可能是 `r3_epilogue` 或結局卡 overlay 沒正確解除，或 `showChoices` 被 pending 狀態鎖住

#### R3 野怪遭遇設計

- [ ] **R3 河城不應有隨機野怪** — 目前 R3 的 `startPatrol()` 會觸發野怪戰鬥，但河城是文明區，不合理。改為：只有玩家主動選擇「去下水道」、「去河岸隧道」、「去城外廢棄森林」時才會觸發巡邏戰鬥。`r3_look` 中樞本身不要有野怪

#### 經濟系統 / 金幣繼承

- [ ] **金幣應限二周目以後獲得** — 一周目不應有金幣來源（或極少），從 NG+ 開始才能獲得少量金幣
- [ ] **金幣可跨周目繼承（像能力值一樣）** — `globalStats.bankedGold` 新欄位，通關後結算保留最佳值，NG+ 開局繼承
- [ ] **灰鶴的欠款應在 NG+ 歸零** — 玩家在 R1/R2 跟灰鶴借的錢，進入下一周目時她「不記得了」，等於免費獲得。這是 lore 友善的攻略獎勵

#### 灰鶴骰子系統 bug

- [ ] **灰鶴輸 60 金後就不賭了** — 目前灰鶴的賭本邏輯寫得太死，輸掉 60 就 cap 住。應該改為每次會面補本金，或讓她「去跟別人賒賬」之類的 in-world 解釋
- [ ] **骰子羅曼史消失** — 之前賭骰子可觸發的羅曼史場景（`r1_crane_drink` / `r2_crane_wound` 之類）現在不會觸發。需要檢查骰子 win/loss 的 `action` 是否還有連到羅曼史節點

#### R3 議會 Boss 風格

- [ ] **「御前守衛」Boss 設計** — 議會廳投票的最終 Boss 改為「高大男人·御前守衛」形象（鎧甲、石化長矛），提升辨識度
